...
static void madera_prop_get_pdata(struct madera_priv *priv)
{
	...
	struct device_node *node = madera->dev->of_node;
	...
	pdata->micbias_regultor = regulator_get(madera->dev, "fsa4480-mic");
	if (IS_ERR(pdata->micbias_regultor)) {
		ret= PTR_ERR(pdata->micbias_regultor);
		dev_err(madera->dev, "Failed to request micbias_regultor: %d\n", ret);
	}

	ret = of_property_read_u32(node, "madera,rgltr-min-voltage", &pdata->rgltr_min_volt);
	if (ret) {
	dev_err(madera->dev, "device %s failed to read madera,rgltr-min-voltage",
		"fsa4480");
	}

	ret = of_property_read_u32(node, "madera,rgltr-max-voltage", &pdata->rgltr_max_volt);
	if (ret) {
	dev_err(madera->dev, "device %s failed to read madera,rgltr-max-voltage",
		"fsa4480");
	}

	ret = of_property_read_u32(node, "madera,rgltr-load-current", &pdata->rgltr_op_mode);
	if (ret) {
	dev_err(madera->dev, "device %s failed to read madera,rgltr-load-current",
		"fsa4480");
	}
	dev_info(madera->dev, "fsa4480 micbias success to enable \n");
...
int audio_soc_util_regulator_disable(struct madera_priv *priv)

{
	...
	if (!pdata )
		return -EINVAL;
	dev = madera->dev;
	rgltr = pdata->micbias_regultor;
	if (!rgltr) {
		dev_err(dev, "Invalid NULL parameter");
		return -EINVAL;
	}

	rc = regulator_disable(rgltr);
	if (rc) {
		dev_err(dev, "%s regulator disable failed", "micbias");
		return rc;
	}
	...
	if (regulator_count_voltages(rgltr) > 0) {
		...
		regulator_set_voltage(rgltr, 0, pdata->rgltr_max_volt);
	}

	return rc;
}

int audio_soc_util_regulator_enable(struct madera_priv *priv)
{
	int32_t rc = 0;
	struct madera *madera = priv->madera;
	struct madera_codec_pdata *pdata = &madera->pdata.codec;
	...
	if (regulator_count_voltages(rgltr) > 0) {
		dev_err(dev, "voltage min=%d, max=%d",
			pdata->rgltr_min_volt, pdata->rgltr_max_volt);

		rc = regulator_set_voltage(
			rgltr, pdata->rgltr_min_volt, pdata->rgltr_max_volt);
		if (rc) {
			dev_err(dev, "%s set voltage failed", "micbias");
			return rc;
		}

		rc = regulator_set_load(rgltr, pdata->rgltr_op_mode);
		if (rc) {
			dev_err(dev, "%s set optimum mode failed",
				"micbias");
			return rc;
		}
	}
	...
	if (rc) {
		dev_err(dev, "%s regulator_enable failed", "micbias");
		return rc;
	}
...
int fsa4480_micbias_demux_put(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
...
/*	struct snd_soc_dapm_context *dapm =
	...
	if(1 == mux){
		audio_soc_util_regulator_disable(priv);
	}else if (0 == mux){
		audio_soc_util_regulator_enable(priv);
	}

	return 0;
}
EXPORT_SYMBOL_GPL(fsa4480_micbias_demux_put);

int fsa4480_micbias_demux_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
...
EXPORT_SYMBOL_GPL(fsa4480_micbias_demux_get);
				 ...
				 struct snd_soc_dai *dai)
{
	...
	dai_priv->sample_rate = params_rate(params);
	dai_priv->bit_width = snd_pcm_format_width(params_format(params));
