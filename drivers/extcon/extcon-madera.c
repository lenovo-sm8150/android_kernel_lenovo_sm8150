...
#if 0
struct madera_extcon *sw_jackdet_info = NULL;
static int sw_jack_status = 0;
#endif
...
int madera_micd_mic_reading(struct madera_extcon *info, int val)
{
	 ...
	 */
	if (((info->micd_ranges[0].max > 30) || (ohms > info->micd_ranges[0].max)) &&
	    (info->num_micd_modes > 1)) {
		if (info->jack_flips >= info->num_micd_modes * 10) {
			dev_dbg(info->dev, "Detected HP/line\n");
			goto done;
		} else {
			fsa4480_switch_event(info->fsa_handle, FSA_MIC_GND_SWAP);
...
out:
	...
	mutex_unlock(&info->lock);
	dev_info(info->dev, "%s: unlock mutex\n",__func__);
...
#if 0
int madera_sw_jackdet(int status)
{
	struct madera_extcon *info = sw_jackdet_info;
	struct madera *madera = info->madera;
	unsigned int val, mask;
	bool cancelled_state;
	int i, present;
	if (!sw_jackdet_info) {
		dev_err(info->dev, "madera_extcon info not initialized!\n");
		return -1;
	}

	dev_dbg(info->dev, "SW jackdet IRQ");
	cancelled_state = madera_jds_cancel_timeout(info);

	pm_runtime_get_sync(info->dev);

	mutex_lock(&info->lock);
	sw_jack_status = status;
	val = status;
	present = status;
	if (present < 0) {
		mutex_unlock(&info->lock);
...
EXPORT_SYMBOL_GPL(madera_sw_jackdet);
#endif
...
static int madera_extcon_probe(struct platform_device *pdev)
{
	...
	dev_err(&pdev->dev, "madera_extcon_probe ==>\n");
	...
	if(NULL != madera->dev){
		if(NULL != madera->dev->of_node){
			info->fsa_handle = of_parse_phandle(node,
							"fsa4480-i2c-handle", 0);
			if (!info->fsa_handle)
				dev_err(&pdev->dev,
					"property %s not detected in node \n",
					"fsa4480-i2c-handle");
			}else{
			dev_err(&pdev->dev,"pdev->dev.of_node is null \n");
	}}
	else{
	dev_err(&pdev->dev,"pdev->dev is null \n");
	}
...
#if 0
	/* Enable the SW jack det */
	regmap_update_bits(madera->regmap, MADERA_IRQ1_MASK_7,
			   0xf, 0xf);
	sw_jackdet_info = info;
	fsa4480_codec_set_callback(madera_sw_jackdet);
	dev_dbg(&pdev->dev, "sw_jackdet_info initialized! \n");
#endif
	return 0;
