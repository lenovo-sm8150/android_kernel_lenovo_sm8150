...
#if FTS_FOD_EN
            input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, data->finger_down);
#else
...
#if FTS_FOD_EN
    data->finger_down = 2;
    ret = fts_FOD_readdata(data, NULL);
    if (1 == ret) {
        data->finger_down = 10;
    } else if (ret < 0) {
        FTS_INFO("In suspend state\n");
    }
#endif
...
static void fts_ts_defer_probe(struct work_struct *work)
{
    int ret = 0;
    struct fts_ts_data *ts_data = container_of(work,
			struct fts_ts_data, ts_probe_work);

    FTS_FUNC_ENTER();
/*    init_completion(&ts_data->drm_init_done);
    ret = wait_for_completion_interruptible_timeout(&ts_data->drm_init_done,
		msecs_to_jiffies(2000));
    if (ret < 0) {
	FTS_ERROR("Wait for DRM init was interrupted\n");
	goto err_drm_init_wait;
    } else if (ret == 0)
	FTS_ERROR("Wait for DRM init timeout\n");
*/
    if (dsi_display_parse_panel() != 1) {
	FTS_ERROR("fts no used\n");
	goto err_drm_init_wait;
    }
    ...
    ret = fts_ex_mode_init(ts_data->client);
    if (ret) {
...
#if FTS_TEST_EN
    ret = fts_test_init(ts_data->client);
    if (ret) {
...
#if FTS_FOD_EN
    ret = fts_i2c_write_reg(ts_data->client, FTS_REG_FOD_ADDRESS, FTS_REG_FOD_EN_VALUE);
    if (ret) {
        FTS_ERROR("init gesture fail");
    }
#endif
...
err_drm_init_wait:
#ifdef CONFIG_FB
     msm_drm_unregister_client(&ts_data->drm_notif);
#endif
    kfree(fts_data->rbuf);//i2c
    if (ts_data->ts_workqueue)
        destroy_workqueue(ts_data->ts_workqueue);
    devm_kfree(&ts_data->client->dev, ts_data);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    INIT_WORK(&ts_data->ts_probe_work, fts_ts_defer_probe);
    queue_work(ts_data->ts_workqueue, &ts_data->ts_probe_work);
