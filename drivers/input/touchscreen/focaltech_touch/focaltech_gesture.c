...
#define KEY_FOD_TOUCH                           0x20
#define KEY_FOD_TOUCH_LEAVE                     0x20
...
static void fts_gesture_en(int enable)
{
    struct input_dev *input_dev = fts_data->input_dev;
    mutex_lock(&input_dev->mutex);
    if (enable == 1) {
...
static struct tp_dev fts_ops = {
	.update_status = fts_gesture_en,
};
...
int fts_FOD_readdata(struct fts_ts_data *ts_data, u8 *data)
{
    ...
    ret = fts_i2c_read_reg(ts_data->client, FTS_REG_FOD_ADDRESS, &state);
    if ((ret < 0) || (state != FTS_REG_FOD_EN_VALUE)) {
        FTS_DEBUG("FOD not enable, don't process FOD");
	 return 0;
    } else if (state == FTS_REG_FOD_EN_VALUE) {
        ret = fts_i2c_read_reg(ts_data->client, FTS_REG_GESTURE_OUTPUT_ADDRESS, &state);
        FTS_DEBUG("[FOD]Ret = %d,Gesture ID =0x%x", ret, state);
	 if (state == 0x26) {
		FTS_INFO("FTS FOD MATCH\n");
		return 1;
	 } else if (state == 0x27) {
		FTS_INFO("FTS FOD LEAVE\n");
		return 0;
	 }
   }
   return 0;
