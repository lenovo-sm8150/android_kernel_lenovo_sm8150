...
static int madera_req_mclk_enable(struct madera *madera,
				     bool enable)
{
	int ret = 0;

	if (enable) {
		ret = clk_prepare_enable(madera->s_clk);
		if (ret) {
			dev_err(madera->dev, "%s: ext clk enable failed\n",
				__func__);
		}
	} else {
		clk_disable_unprepare(madera->s_clk);
	}
...
int madera_dev_init(struct madera *madera)
{
	...
	dev_info(madera->dev, "success to enable DCVDD: %d\n", ret);
#else
	madera->power_en_gpio= of_get_named_gpio(node, "power_en_gpio-gpios", 0);
	if (madera->power_en_gpio < 0) {
		dev_err(madera->dev, "Unable to get \"power_en_gpio\"\n");
		madera->power_en_gpio = -2;
	}
	else {
		dev_info(madera->dev, "madera->power_en_gpio = %d\n", madera->power_en_gpio);
	}

	if (gpio_is_valid(madera->power_en_gpio)) {
		if (0 == gpio_request(madera->power_en_gpio, "MADERA_POWER")) {
			gpio_direction_output(madera->power_en_gpio, 1);
		}
		else {
			dev_err(madera->dev, "madera_POWER: gpio_request fail\n");
		}
	}
	else {
		dev_err(madera->dev, "madera->power_en_gpio is invalid: %d\n", madera->power_en_gpio);
	}
	dev_info(madera->dev, "success to enable DCVDD 1.2: %d\n", ret);
	madera->power_en_gpio_1p8= of_get_named_gpio(node, "power_en_gpio_1p8-gpios", 0);
	if (madera->power_en_gpio_1p8 < 0) {
		dev_err(madera->dev, "Unable to get \"power_en_gpio_1p8\"\n");
		madera->power_en_gpio_1p8 = -2;
	}
	else {
		dev_info(madera->dev, "madera->power_en_gpio_1p8 = %d\n", madera->power_en_gpio_1p8);
	}

	if (gpio_is_valid(madera->power_en_gpio_1p8)) {
		if (0 == gpio_request(madera->power_en_gpio_1p8, "MADERA_POWER_1p8")) {
			gpio_direction_output(madera->power_en_gpio_1p8, 1);
		}
		else {
			dev_err(madera->dev, "madera_POWER: gpio_request fail\n");
		}
		...
		dev_err(madera->dev, "madera->power_en_gpio_1p8 is invalid: %d\n", madera->power_en_gpio_1p8);
	}
	dev_info(madera->dev, "success to enable DCVDD 1p8 %d\n", ret);
	if (IS_ERR(pinctrl)) {
		dev_err(dev, "failed to get pinctrl\n");
		return PTR_ERR(pinctrl);
	};
	pins_enable = pinctrl_lookup_state(pinctrl, "enable");
	if (IS_ERR(pins_enable)) {
		ret = PTR_ERR(pins_enable);
		dev_err(dev, "Could not get active pinstates, err:%d\n", ret);
	};

	ret = pinctrl_select_state(pinctrl, pins_enable);
#endif
	madera_disable_hard_reset(madera);

	/* Register for Clock */
	ext_clk = clk_get(madera->dev, "madera_mclk1");
	if (IS_ERR(ext_clk)) {
		dev_err(madera->dev, "%s: clk get %s failed\n",
			__func__, "madera_clk");
	}

	madera->s_clk = ext_clk;
	madera_req_mclk_enable(madera, true);
